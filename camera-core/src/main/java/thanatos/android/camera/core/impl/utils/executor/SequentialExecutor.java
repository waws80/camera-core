...
package thanatos.android.camera.core.impl.utils.executor;
...
final class SequentialExecutor implements Executor {
    ...
    public void execute(final Runnable task) {
        ...
        synchronized (mQueue) {
            ...
            if (mWorkerRunningState == WorkerRunningState.RUNNING || mWorkerRunningState == WorkerRunningState.QUEUED) {
                mQueue.add(task);
                return;
            }
            ...
            mWorkerRunningState = WorkerRunningState.QUEUING;
        }
        ...
        try {
            ...
            synchronized (mQueue) {
                boolean removed =
                        (mWorkerRunningState == WorkerRunningState.IDLE || mWorkerRunningState == WorkerRunningState.QUEUING)
        ...
        boolean alreadyMarkedQueued = mWorkerRunningState != WorkerRunningState.QUEUING;
        if (alreadyMarkedQueued) {
            return;
        }
        synchronized (mQueue) {
            if (mWorkerRunCount == oldRunCount && mWorkerRunningState == WorkerRunningState.QUEUING) {
                mWorkerRunningState = WorkerRunningState.QUEUED;
            }
        }
    }
    ...
    final class QueueWorker implements Runnable {
        ...
        public void run() {
            try {
                ...
                synchronized (mQueue) {
                    mWorkerRunningState = WorkerRunningState.IDLE;
                }
                throw e;
                    ...
                    synchronized (mQueue) {
                        ...
                        if (!hasSetRunning) {
                            if (mWorkerRunningState == WorkerRunningState.RUNNING) {
                                ...
                                mWorkerRunCount++;
                                mWorkerRunningState = WorkerRunningState.RUNNING;
                                hasSetRunning = true;
                            }
                        }
                        task = mQueue.poll();
