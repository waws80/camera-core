...
import thanatos.android.camera.core.CameraX.LensFacing;
...
import thanatos.android.camera.core.impl.utils.futures.AsyncFunction;
import thanatos.android.camera.core.impl.utils.futures.FluentFuture;
...
public class ImageCapture extends UseCase {
    ...
    boolean is3AConverged(CameraCaptureResult captureResult) {
        ...
        boolean isAfReady = (captureResult.getAfMode() == CameraCaptureMetaData.AfMode.ON_CONTINUOUS_AUTO
                || captureResult.getAfMode() == CameraCaptureMetaData.AfMode.OFF
                || captureResult.getAfMode() == CameraCaptureMetaData.AfMode.UNKNOWN
                || captureResult.getAfState() == CameraCaptureMetaData.AfState.FOCUSED
                || captureResult.getAfState() == CameraCaptureMetaData.AfState.LOCKED_FOCUSED
                || captureResult.getAfState() == CameraCaptureMetaData.AfState.LOCKED_NOT_FOCUSED);

        // Unknown means cannot get valid state from CaptureResult
        boolean isAeReady = captureResult.getAeState() == CameraCaptureMetaData.AeState.CONVERGED
                || captureResult.getAeState() == CameraCaptureMetaData.AeState.UNKNOWN;

        // Unknown means cannot get valid state from CaptureResult
        boolean isAwbReady = captureResult.getAwbState() == CameraCaptureMetaData.AwbState.CONVERGED
                || captureResult.getAwbState() == CameraCaptureMetaData.AwbState.UNKNOWN;
     ...
     * triggered. Trigger AF scan only in {@link CameraCaptureMetaData.AfMode#ON_MANUAL_AUTO} and current AF state is
     * {@link CameraCaptureMetaData.AfState#INACTIVE}. If the AF mode is {@link CameraCaptureMetaData.AfMode#ON_MANUAL_AUTO} and AF state is not
    ...
    void triggerAfIfNeeded(TakePictureState state) {
        if (mEnableCheck3AConverged
                && state.mPreCaptureState.getAfMode() == CameraCaptureMetaData.AfMode.ON_MANUAL_AUTO
                && state.mPreCaptureState.getAfState() == CameraCaptureMetaData.AfState.INACTIVE) {
            triggerAf(state);
        }
    }
    ...
    private final class ImageCaptureRequest {
        ...
        ImageCaptureRequest(
                ...
                @ImageOutputConfig.RotationValue int rotationDegrees,
                ...
                boolean posted =
                        mHandler.post(
                                new Runnable() {
                                    @Override
                                    public void run() {
                                        ImageCapture.ImageCaptureRequest.this.dispatchImage(image);
                                    }
                                });
        ...
        void callbackError(final ImageCaptureError imageCaptureError, final String message,
            ...
            if (mHandler != null && Looper.myLooper() != mHandler.getLooper()) {
                boolean posted = mHandler.post(new Runnable() {
                    @Override
                    public void run() {
                        ImageCapture.ImageCaptureRequest.this.callbackError(imageCaptureError, message, cause);
                    }
                });
                if (!posted) {
                    Log.e(TAG, "Unable to post to the supplied handler.");
